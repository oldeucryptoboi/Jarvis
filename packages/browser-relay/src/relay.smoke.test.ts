 ...
 *
 * Run with:  pnpm --filter @karnevil9/browser-relay test -- relay.smoke
 */
...
describe.skipIf(!available)("relay smoke test (real Chromium via HTTP)", () => {
  ...
  it("takes an accessibility snapshot", async () => {
    ...
    expect(result.snapshot).toContain("Hello KarnEvil9");
  });
  ...
  it("gets text from a specific heading", async () => {
    const result = await post("/actions", {
      action: "get_text",
      target: { role: "heading", name: "Hello KarnEvil9" },
    }) as any;
    expect(result.success).toBe(true);
    expect(result.text).toContain("Hello KarnEvil9");
  });
